...
# класс для определения компонентов графического интерфейса
class Quiz:
    ...
    def __init__(self):
        ...
        # установка номера вопроса равным 0
        self.q_no = 0

        # присваивание запроса функции display_question для последующего обновления.
        self.display_title()
        self.display_question()

        # opt_selected содержит целое значение, которое используется для
        # выбранного варианта в вопросе.
        self.opt_selected = IntVar()

        # отображение переключателя для текущего вопроса и используется для
        # отображения параметров для текущего вопроса
        self.opts = self.radio_buttons()

        # параметры отображения для текущего вопроса
        self.display_options()

        # отображает кнопку для следующего вопроса и выхода.
        self.buttons()

        # нет вопросов
        self.data_size = len(question)

        # ввод счетчик правильных ответов
        self.correct = 0

    # Этот метод используется для отображения результата
    # Подсчитывается количество правильных и неправильных ответов
    # а затем отображает их в конце в виде окна сообщения
    ...
    # Этот метод используется для проверки ответа на
    # текущий вопрос путем вызова check_ins и question no.
    # если вопрос правильный, это увеличивает количество на 1
    # а затем увеличьте номер вопроса на 1. Если это последний
    # вопрос затем он вызывает display result, чтобы отобразить окно сообщения.
    # в противном случае отображается следующий вопрос.
    def next_btn(self):
    ...
    # Первый - это next_button, который переходит к следующему вопросу
    # У него есть свойства, такие как
    # размер, цвет и свойства текста, отображаемого на кнопке. Тогда это
    # указывает, где разместить кнопку на экране. Второй
    # button - это кнопка выхода, которая используется для закрытия графического интерфейса без
    # завершение теста.
    def buttons(self):
    ...
    # Этот метод отменяет выбор переключателя на экране
    # Затем он используется для отображения опций, доступных для текущего
    # вопроса, который мы получаем с помощью номера вопроса и обновлений
    # каждый из вариантов для текущего вопроса переключателя.
    ...
    def display_title(self):
        ...
        title = Label(gui, text="Тест",
                      width=50, bg="green", fg="white", font=("ariel", 20, "bold"))

        # место названия
        title.place(x=0, y=2)

    # Этот метод показывает переключатели для выбора вопроса
    # на экране в указанном положении. Он также возвращает
    # список переключателей, которые позже будут использоваться для добавления опций
